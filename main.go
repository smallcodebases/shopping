...
func main_serve() error {
	...
	if err != nil {
		return fmt.Errorf("setting journal mode to WAL: %w", err)
	}
	...
	for _, entry := range entries {
		...
		if err != nil {
			return fmt.Errorf("parsing %v as int: %w", entry, err)
		}
...
func (handler *Handler) InternalServerError(err error) {
	handler.logger.Error("Unexpected error", "error", err, "stack", string(debug.Stack()))
