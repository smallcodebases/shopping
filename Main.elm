...
type Msg
    ...
    | AddItemNameToListAndStore StoreId String
    | ClearLongpressed
...
update msg model =
    case msg of
        ...
        AddItemNameToListAndStore storeId name ->
            handleAddItemNameToListAndStore model storeId name
    ...
    case Dict.get itemId effectiveModel.items of
        Just item ->
            pure model1
                |> andThenIf
                    (not item.onList)
                    (\model2 -> enqueueRequest (RequestMoveItemOn { item = item.id }) model2)
...
handleAddItemNameToListAndStore : Model -> StoreId -> String -> Eff Model Msg
handleAddItemNameToListAndStore model store name =
    let
        name1 =
            String.trim name
        ...
        StorePage storeId ->
            if model.storePageTab then
                ...
                viewFabWith
                    model
                    "milk"
                    (AddItemNameToListAndStore storeId)
                    -- FIXME we can be more efficient than AddItemToStore here
                    (fabItemSuggestions model (\item -> AddItemNameToListAndStore storeId item.name))
